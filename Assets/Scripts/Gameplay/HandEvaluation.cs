...
public class HandEvaluation : MonoBehaviour
{
	...
	public int GetNumberOfCardsNeededToMakeAStraight()
	{
		return baseCardsNeededToMakeAStraight - Baubles.instance.GetImpactInt("DecreaseCardsNeededForStraight");
	}
	
	public int GetNumberOfCardsNeededToMakeAFlush()
	{
		return baseCardsNeededToMakeAFlush - Baubles.instance.GetImpactInt("DecreaseCardsNeededForFlush");
	}
	
	public int GetNumberOfCardsNeededToMakeAStraightFlush()
	{
		return baseCardsNeededToMakeAStraightFlush - Baubles.instance.GetImpactInt("DecreaseCardsNeededForStraightFlush");
	}

    public void EvaluateHand(List<CardData> hand, bool evaluatingOnlyCardsUsed, HandScoring handScoring = null)
	{
		hand.Sort((x, y) =>
		{
		...
		if(hand.Count >= baseCardsNeededToMakeAFlush - Baubles.instance.GetImpactInt("DecreaseCardsNeededForFlush") || hand.Count >= baseCardsNeededToMakeAStraightFlush - Baubles.instance.GetImpactInt("DecreaseCardsNeededForStraightFlush"))
		{
			List<int> foundFlushes = new List<int>();
			List<int> foundPossibleStraightFlushes = new List<int>();
			for(int i = 0; i < 4; i++)
			{
				if(numberOfEachSuit[i] + numberOfEachSuit[4] >= baseCardsNeededToMakeAFlush - Baubles.instance.GetImpactInt("DecreaseCardsNeededForFlush"))
				{
					flush = true;
					foundFlushes.Add(i);
				}
				if(numberOfEachSuit[i] + numberOfEachSuit[4] >= baseCardsNeededToMakeAStraightFlush - Baubles.instance.GetImpactInt("DecreaseCardsNeededForStraightFlush"))
				{
			...
			for(int i = 0; i < foundPossibleStraightFlushes.Count; i++)
			{
				...
				List<CardData> straightFlushCardsToCheck = DoesHandContainStraight(flushToCheck, Baubles.instance.GetImpactInt("IncreaseMaxGapInStraights"), baseCardsNeededToMakeAStraightFlush - Baubles.instance.GetImpactInt("DecreaseCardsNeededForStraightFlush"), Baubles.instance.GetImpactInt("StraightsCanWrap") > 0 ? true : false);
		...
		bool straight = false;
		List<CardData> straightCards = DoesHandContainStraight(hand, Baubles.instance.GetImpactInt("IncreaseMaxGapInStraights"), baseCardsNeededToMakeAStraight - Baubles.instance.GetImpactInt("DecreaseCardsNeededForStraight"), Baubles.instance.GetImpactInt("StraightsCanWrap") > 0 ? true : false);
		...
		for(int i = 17; i >= 0; i--)
		{
			if(handsContained[i])
			{
				if(i >= 14)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(hand, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(hand, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				else if(i >= 10)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(sixHand, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(sixHand, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				else if(i == 9)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(fiveOfAKindCards, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(fiveOfAKindCards, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				if(i == 8)
				{
					if(royalFlush)
					{
						if(handScoring == null)
						{
							PlayArea.instance.HandEvaluated(straightFlushCards, evaluatingOnlyCardsUsed, handsContained, true);
						}
						else
						{
							handScoring.HandEvaluated(straightFlushCards, evaluatingOnlyCardsUsed, handsContained, true);
						}
					}
					else
					{
						if(handScoring == null)
						{
							PlayArea.instance.HandEvaluated(straightFlushCards, evaluatingOnlyCardsUsed, handsContained);
						}
						else
						{
							handScoring.HandEvaluated(straightFlushCards, evaluatingOnlyCardsUsed, handsContained);
						}
					}
				}
				else if(i == 7)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(fourOfAKindCards, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(fourOfAKindCards, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				else if(i == 6)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(fullHouseCards, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(fullHouseCards, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				else if(i == 5)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(flushCards, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(flushCards, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				else if(i == 4)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(straightCards, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(straightCards, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				else if(i == 3)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(threeOfAKindCards, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(threeOfAKindCards, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				else if(i == 2)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(twoPairCards, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(twoPairCards, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				else if(i == 1)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(onePairCards, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(onePairCards, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				else if(i == 0)
				{
					if(handScoring == null)
					{
						PlayArea.instance.HandEvaluated(highCardCards, evaluatingOnlyCardsUsed, handsContained);
					}
					else
					{
						handScoring.HandEvaluated(highCardCards, evaluatingOnlyCardsUsed, handsContained);
					}
				}
				return;
			}
		}
		LocalInterface.instance.DisplayError("EvaluateHand reached endpoint with no hand evaluated");
	}
